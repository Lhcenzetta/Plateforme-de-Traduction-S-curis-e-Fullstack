...
@router.post("/register", response_model=schemas.UserCreat)
def register(user: schemas.UserCreat, db: Session = Depends(get_db)):
    new_user = User(
...
@router.post("/login")
def login(form_data: schemas.Usercheck, db: Session = Depends(get_db)):
